...
static const struct ovsrec_snmpv3_user* find_snmpv3_user(const struct ovsdb_idl *idl, const char *username){
...
static int add_auth_priv_info(netsnmp_session *session, const struct ovsrec_snmpv3_user *user_row){
    ...
    if(auth_flag){
        ...
        else if(strcmp(auth_protocol, OVSREC_SNMPV3_USER_AUTH_PROTOCOL_SHA) == 0){
            session->securityAuthProto = usmHMACSHA1AuthProtocol;
            session->securityAuthProtoLen = USM_AUTH_PROTO_SHA_LEN;
        }
        else{
            VLOG_ERR("Invalid auth protocol for snmpv3 user: %s", auth_protocol);
            return -1;
        }
    }
    ...
    if(priv_flag){
        ...
        else if(strcmp(priv_protocol, OVSREC_SNMPV3_USER_PRIV_PROTOCOL_AES) == 0){
    ...
    if(auth_flag){
        ...
        if(generate_Ku(session->securityAuthProto, session->securityAuthProtoLen, (u_char*)auth_pass_phrase, strlen(auth_pass_phrase), session->securityAuthKey, &session->securityAuthKeyLen) != SNMPERR_SUCCESS){
            VLOG_ERR("Error generating a key (Ku) from the supplied authentication pass phrase:%s", auth_pass_phrase);
            return -1;
        }
    }
    ...
    if(priv_flag){
        ...
        if(generate_Ku(session->securityAuthProto, session->securityAuthProtoLen, (u_char*)priv_pass_phrase, strlen(priv_pass_phrase), session->securityPrivKey, &session->securityPrivKeyLen) != SNMPERR_SUCCESS){
            VLOG_ERR("Error generating a key (Ku) from the supplied privacy pass phrase:%s", priv_pass_phrase);
            return -1;
        }
    }
...
static int add_snmpv3_user(const struct ovsdb_idl *idl, netsnmp_session *session, const struct ovsrec_snmp_trap* trap_row){
    if(global_snmp_engine_id == NULL){
    ...
    if(!snmp_hex_to_binary(&ebuf, &ebuf_len, &eout_len, 1, global_snmp_engine_id)){
    ...
    const struct ovsrec_snmpv3_user *user_row = find_snmpv3_user(idl, trap_row->community_name);
    if(user_row == NULL){
...
static int add_trap_community(netsnmp_session *sess, const struct ovsrec_snmp_trap* trap_row){
    ...
    sess->community = (u_char*)trap_community;
    sess->community_len = strlen(trap_community);
    return 0;
}
...
int ops_snmp_send_trap(struct ovsdb_idl*idl, netsnmp_pdu *pdu){
    ...
    if(trap_row == NULL){
        VLOG_ERR("ovsrec_snmp_trap_first failed to return trap row\n");
        return -1;
    }
    ...
    OVSREC_SNMP_TRAP_FOR_EACH(trap_row, idl){
        ...
        netsnmp_pdu *temp_pdu = snmp_clone_pdu(pdu);
        memcpy(temp_pdu, pdu, sizeof(netsnmp_pdu)/sizeof(char));
        ...
        if(strcmp(trap_type, OVSREC_SNMP_TRAP_TYPE_INFORM) == 0){
            inform = 1;
            temp_pdu->command = SNMP_MSG_INFORM;
        }

        if(strcmp(trap_version, OVSREC_SNMP_TRAP_VERSION_V1) == 0){
            session.version = SNMP_VERSION_1;
            temp_pdu->command = SNMP_MSG_TRAP;
            if(add_trap_community(&session, trap_row)<0){
                VLOG_ERR("Failed in add_trap_community");
                status = -1;
                goto loop_cleanup;
            }
        }
            ...
            if(add_snmpv3_user(idl, &session, trap_row) < 0){
                VLOG_ERR("Failed in adding add_snmpv3_user");
                status = -1;
                goto loop_cleanup;
            }
        }
        ...
        if(inform){
            status = snmp_synch_response(ss, temp_pdu, &response);
        }
        else{
            status = snmp_send(ss, temp_pdu) == 0;
        }
loop_cleanup:
        if(status){
            VLOG_ERR(inform ? "snmpinform failed with status:%d" : "snmptrap failed with statis:%d", status);
            if(!inform && temp_pdu != NULL){
                snmp_free_pdu(temp_pdu);
            }
        }
